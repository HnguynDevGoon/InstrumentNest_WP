...
function livesupporti_init() {
	if (isset($_POST['_wpnonce']) && !wp_verify_nonce($_POST['_wpnonce'], 'update-details'))
	{
		die('Security check');
	}
	else
		...
		echo '
				...
				<script type="text/javascript">
				  (function() {
					...
					s.src = "https://app.livesupporti.com/js/clientAsync.js?acc='.$license.'&skin='.$skin.'";
...
function getLiveSupportiAdminMenu() {
	$icon = "https://app.livesupporti.com/Images/favicon.png";
	...
	else
	{
		if (isset($_POST["action"]) && $_POST["action"] == "signupin")
		{
			...
			if ($email && $password != "")
			{
				...
				$lsResponse = json_decode(postLiveSupportiRequest("https://app.livesupporti.com/wordpress/signupsignin", $liveSupportiData));
				if (!isset($lsResponse->wp_error) && !isset($lsResponse->errorEmail) && !isset($lsResponse->errorPassword))
				{
					update_option('liveSupportiEmail', $email);
					update_option('liveSupportiPassword', $password);
					update_option('liveSupportiSkin', sanitize_text_field($_POST['selectLiveSupportiSkin']));
					
					if (!isset($lsResponse->error) && isset($lsResponse->code) && $lsResponse->code != "")
					{
						update_option("liveSupportiLicense", sanitize_text_field($lsResponse->code));
					}
				}
			}
			else
...
<script>
	...
	<div style='padding:15px 15px 15px 0px;'>
		<div style='padding:15px;  height: 630px; background-image:url("<?php echo plugin_dir_url( __FILE__ ).'wordpress-bg.png'; ?>"); box-shadow:0px 2px 10px #878787;'>
			<?php if (get_option('liveSupportiLicense') == "")
			{
			?>
				<form id="form1" name="form1" method="post" action="admin.php?page=livesupporti/livesupporti.php" style="width:100%;margin:0px auto;display:block;">
				<?php wp_nonce_field('update-details') ?>
					...
					<table style="position:relative; border-collapse:collapse;border-spacing:0; width:100%;">
						<tr>
							...
							<td style="margin:0px;padding:0px;width:50%;" align="left">
								<span id="spanError" style="display:block;height:18px;margin:4px 0px 10px -160px;color:#DC4B45;font-weight:600;visibility:<?php if (isset($lsResponse->wp_error) || isset($lsResponse->error)){?>visible;<?php } else { ?>hidden;<?php }?>"><?php echo(esc_html($lsResponse->wp_error));echo(esc_html($lsResponse->error));?></span>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td style="margin:0px;padding:0px;width:50%;" align="left">
								<span id="spanEmailError" style="display:block;height:18px;margin:4px 0px 20px -160px;color:#DC4B45;font-weight:600;visibility:<?php if (isset($lsResponse->errorEmail)){?>visible;<?php } else { ?>hidden;<?php }?>"><?php echo(esc_html($lsResponse->errorEmail));?></span>
							</td>
						</tr>
						<tr>
							<td style="margin:0px;padding:0px;width:50%" align="right">
						...
						<tr>
							...
							<td style="margin:0px;padding:0px;width:50%" align="left">
								<span id="spanPasswordError" style="display:block;height:18px;margin:4px 0px 20px -160px;color:#DC4B45;font-weight:600;visibility:<?php if (isset($lsResponse->errorPassword)){?>visible;<?php } else { ?>hidden;<?php }?>"><?php echo(esc_html($lsResponse->errorPassword));?></span>
							</td>
						</tr>
						<tr>
							<td style="margin:0px;padding:0px;width:50%" align="right">
								...
								<select name="selectLiveSupportiSkin" style="height:40px;margin-left:-160px;border-radius:2px;vertical-align:baseline">
									<?php
										...
										foreach ($skins as $val => $label)
										{
											...
											echo "<option value='".esc_html($val)."' ".esc_html($selected).">".esc_html($label)."</option>\n";
										}
									?>
								</select>
							</td>
			...
			{
			?>
			<form id="form2" name="form2" method="post" action="admin.php?page=livesupporti/livesupporti.php" style="width:640px;margin:0px auto;display:block;">
				<?php wp_nonce_field('update-details') ?>
...
function livesupporti_uninstall_plugin() {
	if (get_option('liveSupportiEmail', false))
	{
		...
		$lsResponse = json_decode(postLiveSupportiRequest("https://app.livesupporti.com/wordpress/uninstall", $liveSupportiData));	
	}
	
}

function postLiveSupportiRequest($url, $_data )
  {
    $args = array(
      "body" => $_data,
